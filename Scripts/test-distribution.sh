...
MACCHANNEL_RELEASE_NOTES="$repo_root/Distribution/ReleaseNotes/v1.2.4.md" \
    bash Scripts/build-distribution.sh
...
grep -F "版本 1.2.4 (17)" "$mounted_path/README.txt" >/dev/null
...
test "$(plutil -extract version raw -o - $test_dist/DropMesh.manifest.json)" = 1.2.4
test "$(plutil -extract build raw -o - $test_dist/DropMesh.manifest.json)" = 17

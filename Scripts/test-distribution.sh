...
grep -F "版本 1.0.0 (1)" "$mounted_path/README.txt" >/dev/null
...
test "$(plutil -extract version raw -o - dist/MacChannel.manifest.json)" = 1.0.0
test "$(plutil -extract build raw -o - dist/MacChannel.manifest.json)" = 1

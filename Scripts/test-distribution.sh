...
grep -F "版本 1.2.6 (21)" "$mounted_path/README.txt" >/dev/null
...
test "$(plutil -extract CFBundleVersion raw -o - "$plist")" = 21
...
test "$(plutil -extract build raw -o - $test_dist/DropMesh.manifest.json)" = 21

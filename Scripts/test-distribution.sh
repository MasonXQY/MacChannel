...
grep -F "版本 1.1.6 (8)" "$mounted_path/README.txt" >/dev/null
codesign --verify --deep --strict --verbose=2 "$mounted_path/MacChannel.app"

plist="$mounted_path/MacChannel.app/Contents/Info.plist"
test "$(plutil -extract CFBundleShortVersionString raw -o - "$plist")" = 1.1.6
test "$(plutil -extract CFBundleVersion raw -o - "$plist")" = 8
...
test "$(plutil -extract version raw -o - dist/MacChannel.manifest.json)" = 1.1.6
test "$(plutil -extract build raw -o - dist/MacChannel.manifest.json)" = 8

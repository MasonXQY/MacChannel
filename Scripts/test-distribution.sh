...
MACCHANNEL_RELEASE_NOTES="$repo_root/Distribution/ReleaseNotes/v1.2.5.md" \
    bash Scripts/build-distribution.sh
...
grep -F "版本 1.2.5 (18)" "$mounted_path/README.txt" >/dev/null
...
test "$(plutil -extract CFBundleShortVersionString raw -o - "$plist")" = 1.2.5
test -n "$(plutil -extract NSDownloadsFolderUsageDescription raw -o - "$plist")"
test "$(plutil -extract CFBundleVersion raw -o - "$plist")" = 18
...
test "$(plutil -extract version raw -o - $test_dist/DropMesh.manifest.json)" = 1.2.5
test "$(plutil -extract build raw -o - $test_dist/DropMesh.manifest.json)" = 18

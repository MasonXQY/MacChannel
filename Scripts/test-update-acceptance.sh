...
security list-keychains -d user >"$test_root/keychains.before"
chmod 600 "$test_root/keychains.before"
signing_keychain="$(security login-keychain | \
...
stop_server() {
    ...
    if kill -0 "$server_pid" 2>/dev/null; then
        ...
        for _ in {1..40}; do kill -0 "$server_pid" 2>/dev/null || break; sleep 0.05; done
        if kill -0 "$server_pid" 2>/dev/null; then
            kill -KILL -"$server_pid" 2>/dev/null || true
            for _ in {1..40}; do kill -0 "$server_pid" 2>/dev/null || break; sleep 0.05; done
        fi
    fi
    set +e; wait "$server_pid" 2>/dev/null; local wait_status=$?; set -e
    if kill -0 "$server_pid" 2>/dev/null; then
    ...
    [[ "$wait_status" -eq 0 || "$wait_status" -eq 143 || "$wait_status" -eq 137 ]]
}
cleanup() {
    local exit_code=$?
    ...
    security list-keychains -d user >"$test_root/keychains.after"
...
openssl pkey -in "$test_root/sparkle-a.pem" -outform DER 2>/dev/null | tail -c 32 | \
...
openssl pkey -in "$test_root/sparkle-b.pem" -outform DER 2>/dev/null | tail -c 32 | \
...
assert_embedded_team() {
    ...
    for code in "${signed_code[@]}"; do
        ...
        [[ "$team" == "$expected_team" ]]
        codesign -d -r- "$code" 2>&1 | grep -F 'designated =>' >/dev/null
    done
    codesign --verify --deep --strict "$app"
    if codesign -d --entitlements - "$app/Contents/MacOS/MacChannelApp" 2>&1 | \
    ...
    for code in "$app/Contents/MacOS/MacChannelUpdateAcceptance" \
        ...
        codesign -d --entitlements - "$code" 2>&1 | \
...
! plutil -extract MacChannelUpdateTestSigner raw -o - "$production_app/Contents/Info.plist" \
    >/dev/null 2>&1
! codesign -d --entitlements - "$production_app/Contents/MacOS/MacChannelApp" 2>&1 | \
    grep -F 'com.apple.security.cs.disable-library-validation' >/dev/null
...
prepare_app() {
    ...
    for nested_code in \
        ...
        [[ ! -e "$nested_code" ]] || \
            codesign "${signing_args[@]}" "$nested_code" >>"$signing_log" 2>&1
    done
    codesign "${signing_args[@]}" "$sparkle" >>"$signing_log" 2>&1
    codesign "${signing_args[@]}" "$destination/Contents/MacOS/WebRTC.framework" \
        >>"$signing_log" 2>&1
    ...
    codesign "${signing_args[@]}" "$destination/Contents/MacOS/MacChannelApp" \
        >>"$signing_log" 2>&1
    codesign "${signing_args[@]}" \
...
assert_test_signature_chain() {
    ...
    host_details="$(codesign -dvvv "$app" 2>&1)"
    ...
    for code in "${signed_code[@]}"; do
        [[ ! -e "$code" ]] && continue
        codesign --verify --strict "$code"
        code_details="$(codesign -dvvv "$code" 2>&1)"
...
if [[ "$static_only" == 1 ]]; then
    security list-keychains -d user >"$test_root/keychains.static-final"
...
start_server() {
    ...
    openssl req -x509 -newkey rsa:2048 -nodes -keyout "$case_root/tls.key" \
    ...
    tls_pin="$(openssl x509 -in "$case_root/tls.crt" -outform DER | shasum -a 256 | awk '{print $1}')"
...
run_harness() {
    ...
    run_bounded "$case_root/acceptance.log" 40 env -i PATH="$PATH" HOME="$case_root/home" \
        ...
        MACCHANNEL_UPDATE_TEST_TLS_HOSTNAME=localhost \
...
resign_feed() { "$sign_update" --ed-key-file "$test_root/sparkle-a.key" "$1" >/dev/null; }
...
run_case() {
    ...
    hdiutil create -srcfolder "$case_root/stage" -volname "MacChannel $name" -fs HFS+ \
    ...
    case "$mutation" in
        ...
        wrong-key)
            wrong_signature="$("$sign_update" --ed-key-file "$test_root/sparkle-b.key" -p "$case_root/MacChannel.dmg")"
        ...
        external-release-notes)
            ...
            resign_feed "$case_root/appcast.xml" ;;
        hostname-mismatch|redirect-external|non-https|none) ;;
        *) return 64 ;;
    esac
    ...
    case "$mutation" in
        ...
        hostname-mismatch) grep -F 'state=tls-policy-reject reason=certificate-or-hostname' "$case_root/acceptance.log" >/dev/null ;;
...
hdiutil create -srcfolder "$control_root/stage" -volname 'MacChannel dead control' -fs HFS+ \
    -format UDZO -ov "$control_root/MacChannel.dmg" >/dev/null
start_server "$control_root" localhost
(cd "$control_root" && "$generate_appcast" --ed-key-file "$test_root/sparkle-a.key" \
    --download-url-prefix "https://localhost:$server_port/" --maximum-versions 1 \
...
hdiutil create -srcfolder "$offline_root/stage" -volname 'MacChannel offline' -fs HFS+ \
...
security list-keychains -d user >"$test_root/keychains.final"

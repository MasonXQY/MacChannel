...
if [[ "$update_testing" == 1 && -n "$update_fixture_root" ]]; then
    ...
    if ! env -i PATH="$PATH" HOME="$signing_home" TMPDIR="$signing_tmp" LANG=C LC_ALL=C \
        ...
        MACCHANNEL_UPDATE_STAPLER_COMMAND="${MACCHANNEL_UPDATE_STAPLER_COMMAND:-}" \
        MACCHANNEL_UPDATE_SPCTL_COMMAND="${MACCHANNEL_UPDATE_SPCTL_COMMAND:-}" \
        ...
        MACCHANNEL_UPDATE_TEST_KEY_ACCESS_MARKER="${MACCHANNEL_UPDATE_TEST_KEY_ACCESS_MARKER:-}" \

...
| `submit` | `POST /v1/pairing/{code}/join` | `code, joinRequest` | `202 {sessionID, sessionExpiresAt}` |
| host join poll | `POST /v1/pairing/{code}/host` | `code` | `200 {sessionID, joinRequest, sessionExpiresAt}` |
...
| `remove` | `DELETE /v1/pairing/{code}` | `code` | `204` (host-bound and idempotent) |
...
The service participant-binds each post-join operation to the authenticated host or joiner identity. The code offer expires independently. A successful join creates a fresh five-minute session, while a reserved or committed authorization mailbox has its own fifteen-minute expiry and survives process restart in PostgreSQL mode.

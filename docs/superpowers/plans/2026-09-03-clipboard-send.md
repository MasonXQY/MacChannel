...
Add an AppKit smoke that sends a result through the production `RuntimeReceiveEventSource` and `MacChannelApplicationDelegate` receive-event path into `RecentReceiveStore`, receive notification, and the status-item unread surface. Inject a `StatusItemController` whose `NativeClipboardTransferPreparer` is bound to a uniquely named isolated receiver pasteboard. Assert that receive processing neither calls the preparer nor changes the pasteboard sentinel or change count. Add a fail-closed lexical source contract whose production roots are derived from `Package.swift` and cross-checked against every Swift file under `App/` and `Sources/`. It must ignore pure comment, string-literal, and bare/extended/multiline regex-literal text while recursively scanning executable Swift interpolation expressions, including nested strings, nested regex literals, and balanced parentheses. Bare-regex terminator state must distinguish unescaped trailing whitespace from compiler-valid escaped trailing spaces, including spaces preceded by multiple backslashes, so regex parentheses cannot truncate an enclosing interpolation scan. Normalize Swift backticked identifiers before matching. Treat every executable `.general` token as forbidden except for exactly one explicit `NSPasteboard.general` access in the send adapter `App/ClipboardTransferSource.swift`; shorthand and backticked spelling are forbidden even in that allowlisted file. This conservative policy avoids incomplete type-flow inference and prevents receive/app/notification code from bypassing the injected boundary. Never touch the user's system pasteboard.
...
Expected: direct receive integration passes for TXT and PNG; the full AppKit receive-event path leaves the injected isolated pasteboard untouched and never invokes the explicit-send preparer; lexical mutation fixtures reject qualified, backticked, comment-separated, all shorthand/member `.general`, `Sources/` access, and executable access inside string/regex interpolation while allowing those spellings in pure comments, strings, and regex literal text.

...
Expected: direct receive integration passes for TXT and PNG; the full AppKit receive-event path leaves the injected isolated pasteboard untouched and never invokes the explicit-send preparer; lexical mutation fixtures reject qualified, comment-separated, all shorthand/member `.general`, `Sources/` access, and `.general` inside normal/multiline/raw interpolation while allowing those spellings in pure comments and literal text.

...
审计日期：2026-08-27
被审计代码 commit：`4435dd194e117e935f15b792827af288cd51705e`
静态证据时间（UTC）：`2026-08-27T15:24:41Z`
静态审计脚本 SHA-256：
...
`764e280` 是早期静态审计基线，已被后续门禁修订取代；它不包含下方列出的最终脚本集合，
不得用于复现当前审计结果。
...
包含当前静态证据哈希、删除未受信运行时 PASS 路径、删除旧 evidence scanner，并以状态 2
固定运行时门禁的不可变内容提交为 `f506df56866dcb6dc518cd6153006a66aa2a49ae`。本行由后续仅追加提交记录。

本轮静态审计覆盖的实现提交为
`4435dd194e117e935f15b792827af288cd51705e`；包含本次审计时间与目标提交更新的文档版本
由后续只追加 provenance 提交固定。
